...
class IsJointAccountController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen reportIdAction) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(IsJointAccountPage()) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen reportIdAction).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.setWithReportId(IsJointAccountPage(), value))
            _              <- repository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(IsJointAccountPage(), mode, updatedAnswers))
      )
  }
}
