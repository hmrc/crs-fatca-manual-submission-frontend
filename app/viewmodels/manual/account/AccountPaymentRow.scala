...
package viewmodels.manual.account

import models.manual.account.AccountPayment
import play.api.i18n.Messages

case class AccountPaymentRow(description: String, changeUrl: String, removeUrl: String)
  ...
  def rows(accountPaymentList: Seq[AccountPayment])(implicit messages: Messages) =
    accountPaymentList.zipWithIndex.map {
      case (accountPayment, index) =>
        ...
        AccountPaymentRow(
          description,
          controllers.manual.account.routes.CurrentAccountPaymentIndexController.onChangeRedirect(index).url,
