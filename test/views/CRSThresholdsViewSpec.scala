...
class CRSThresholdsViewSpec extends SpecBase {

  private val application = applicationBuilder().build()

  private val view: CRSThresholdsView                                    = application.injector.instanceOf[CRSThresholdsView]
  ...
  "CRSThresholdsView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("Is the financial institution applying thresholds to any of their accounts in their due diligence process for CRS?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("Is TestFIName applying thresholds to any of their accounts in their due diligence process for CRS?")
