...
class IndividualOrOrganisationViewSpec extends SpecBase {

  private val application = applicationBuilder().build()

  private val view: IndividualOrOrganisationView                         = application.injector.instanceOf[IndividualOrOrganisationView]
  ...
  "IndividualOrOrganisationView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("Is the substantial owner an individual or organisation?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("Is the substantial owner an individual or organisation?")
      }
