...
class UkAddressViewSpec extends SpecBase {
  ...
  "UkAddressView" - {
    "should render page components" - {
      val renderedHtml = view(formProvider(), NormalMode, "Test Name", Countries.ukTerritories)
      ...
      "must display title" in {
        doc.title() must include("What is the account holder’s registered address?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("What is the registered address for Test Name?")
      }
