...
class AccountHolderAddressNonUkViewSpec extends SpecBase {
  ...
  private val view: AccountHolderAddressNonUkView =
  ...
  private val formProvider =
    new AccountHolderAddressNonUkFormProvider()
    ...
    "should render the page components" - {
      ...
      "must display the page title" in {
        doc.title() must include(
          "What is the account holder’s address?"
        )
      }

      "must display the heading containing the account holder name" in {
        doc.select("h1").text() mustBe
          s"What is the address for $individualAccountHolder?"
      }
      ...
      "must submit to the AccountHolderAddressNonUk controller" in {
        val formElement =
        ...
        formElement.attr("action") mustBe
          controllers.manual.accountHolders.routes.AccountHolderAddressNonUkController
            .onSubmit(NormalMode)
            .url
      }
