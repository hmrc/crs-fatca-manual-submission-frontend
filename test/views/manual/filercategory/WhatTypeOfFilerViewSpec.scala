...
class WhatTypeOfFilerViewSpec extends SpecBase {

  private val application = applicationBuilder().build()

  private val view: WhatTypeOfFilerView                                  = application.injector.instanceOf[WhatTypeOfFilerView]
  ...
  "WhatTypeOfFilerView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("What type of filer is the financial institution?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("What type of filer is fiName?")
      }
