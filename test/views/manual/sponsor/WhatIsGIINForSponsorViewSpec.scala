...
class WhatIsGIINForSponsorViewSpec extends SpecBase {
  ...
  "WhatIsGIINForSponsorView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("Global Intermediary Identification Number (GIIN)")
      }

      "must display heading" in {
        doc.select("h1").text() must include("Global Intermediary Identification Number (GIIN)")
      }

      "must display label" in {
        doc.select("label").text() must include("What is the GIIN for TestFIName?")
      }
