...
class IsSponsorBasedInUKViewSpec extends SpecBase {
  ...
  "IsSponsorBasedInUKView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("Is the sponsor based in the UK, Jersey, Guernsey or the Isle of Man?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("Is TestFIName based in the UK, Jersey, Guernsey or the Isle of Man?")
      }
