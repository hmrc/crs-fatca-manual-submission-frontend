...
class CRSDormantAccountsViewSpec extends SpecBase {
  ...
  private val view: CRSDormantAccountsView                               = application.injector.instanceOf[CRSDormantAccountsView]
  ...
  "CRSDormantAccountsView" - {
    ...
    "should render page components" - {
      ...
      "must display title" in {
        doc.title() must include("Is the financial institution treating dormant accounts as not being reportable accounts for CRS?")
      }

      "must display heading" in {
        doc.select("h1").text() must include("Is TestFIName treating dormant accounts as not being reportable accounts for CRS?")
