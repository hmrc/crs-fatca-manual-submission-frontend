...
//      running(application) {
//        when(mockService.getSubmissionHistory(any())(using any()))
...
//
//      running(application) {
//        when(mockService.getSubmissionHistory(any)(using any)).thenReturn(Future.failed(InternalServerException("Failed")))
//        val request = FakeRequest(GET, routes.ReadSubmissionDataController.onPageLoad("fiId", "fiName").url)
