...
class IndividualNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val individualNameRoute = controllers.manual.cpso.routes.IndividualNameController.onSubmit(NormalMode).url
  ...
  "IndividualName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      val validAnswer       = IndividualName("first-name", "last-name")
      implicit val reportId = ReportId(CRS, 2025, None, "TestfiID")
      val userAnswers = ua
        .withPage(CurrentCPSOIdPage()(reportId), currentId)
        .withPage(IndividualNamePage(currentId)(reportId), validAnswer)
