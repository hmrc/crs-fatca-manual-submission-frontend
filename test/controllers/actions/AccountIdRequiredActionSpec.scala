...
class AccountIdRequiredActionSpec extends SpecBase {

  class Harness extends AccountIdRequiredActionImpl {

    def callRefine[A](request: ReportIdRequest[A]): Future[Either[Result, AccountIdRequest[A]]] =
      refine(request)
  }
  ...
  "AccountIdRequiredAction" - {

    "must return a AccountIdRequest when AccountId exists in user answers" in {
      ...
      val userAnswers =
        emptyUserAnswers
          ...
          .success
          .value
          .set(CurrentAccountIdPage()(reportId), accountId)
          .success
          .value
      ...
      result match {
        case Right(request) =>
          ...
          request.accountId mustBe accountId
    ...
    "must redirect to Journey Recovery when CurrentAccountIdPage does not exist in user answers" in {
      val action = new Harness
