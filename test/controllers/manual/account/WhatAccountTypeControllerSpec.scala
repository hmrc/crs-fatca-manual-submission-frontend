...
class WhatAccountTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val whatAccountTypeRoute = controllers.manual.account.routes.WhatAccountTypeController.onPageLoad(NormalMode).url
  val accountId: AccountId      = AccountId(value = "SomeId")
  val formProvider              = new WhatAccountTypeFormProvider()
