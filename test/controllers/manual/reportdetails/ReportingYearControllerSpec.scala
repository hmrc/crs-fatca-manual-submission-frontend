...
import controllers.manual.reportdetails.routes.ReportingYearController
...
class ReportingYearControllerSpec extends SpecBase {
  ...
  private lazy val reportingYearRoute = ReportingYearController.onPageLoad(NormalMode).url
  ...
  "ReportingYear Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, reportingYearRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ReportingYearView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ReportingYearPage, validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, reportingYearRoute)
