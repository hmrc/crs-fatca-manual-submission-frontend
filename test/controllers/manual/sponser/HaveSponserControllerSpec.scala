...
class HaveSponserControllerSpec extends SpecBase with MockitoSugar {
  ...
  "HaveSponser Controller" - {
    ...
    val ua                = emptyUserAnswers.withPage(ReportIdPage, reportId).withPage(FINamePage(), testFiName)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), NormalMode, testFiName)(request, messages(application)).toString
      }
    }
