...
import forms.manual.cpso.IndividualHavePlaceOfBirthFormProvider
...
import pages.manual.cpso.{CurrentCPSOIdPage, IndividualHavePlaceOfBirthPage, IndividualNamePage, IndividualOrOrganisationPage}
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers.*
import views.html.manual.cpso.IndividualHavePlaceOfBirthView
...
class IndividualHavePlaceOfBirthControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val individualHavePlaceOfBirthRoute = controllers.manual.cpso.routes.IndividualHavePlaceOfBirthController.onPageLoad(NormalMode).url
  ...
  "IndividualHavePlaceOfBirth Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, NormalMode, false, name.fullName)(request, messages(application)).toString
      }
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = ua.set(IndividualHavePlaceOfBirthPage(cpsoId, reportId), true).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), NormalMode, false, name.fullName)(request, messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, NormalMode, false, name.fullName)(request, messages(application)).toString
      }
    }
