...
class WhatAccountTypeSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "WhatAccountType" - {

    "must deserialise valid values" in {

      val gen = Gen.oneOf(WhatAccountType.values.toSeq)
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!WhatAccountType.values.map(_.toString).contains(_))
