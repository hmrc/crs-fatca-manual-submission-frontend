...
class SponsorNameSummarySpec extends SpecBase {
  ...
  ".row" - {
    ...
    "must return SummaryListRow with value" in {
      ...
      val summary = SponsorNameSummary.row(ua)

      summary.isDefined mustBe true

      summary.get mustBe SummaryListRowViewModel(
        key = KeyViewModel("sponsorName.checkYourAnswersLabel"),
        value = ValueViewModel("Test Sponsor"),
        actions = Seq(
